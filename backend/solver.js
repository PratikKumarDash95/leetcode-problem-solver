...
function getSolverConfig() {
  ...
  const provider = (
    ...
    (freeModelApiKey ? "freemodel" : openRouterApiKey ? "openrouter" : "openai")
  ).toLowerCase();
...
function formatProviderError(providerName, status, result) {
  ...
  const parts = [
    ...
    error.metadata?.provider_name
      ...
      : "",
    error.metadata?.raw
      ? `raw: ${String(error.metadata.raw).slice(0, 300)}`
      : "",
  ].filter(Boolean);
...
async function generateSolution(prompt) {
  ...
  const apiKey = useFreeModel
    ? config.freeModelApiKey
    : useOpenAI
      ? config.openAiApiKey
      : config.openRouterApiKey;
  const model = useFreeModel
    ? config.freeModelModel
    : useOpenAI
      ? config.openAiModel
      : config.openRouterModel;
  const providerName = useFreeModel
    ? "FreeModel"
    : useOpenAI
      ? "OpenAI"
      : "OpenRouter";
  ...
  const systemPrompt =
    "Return only the completed LeetCode solution code. Do not include explanations.";
  const body = {
    model,
    messages: [
  ...
  try {
    ...
    throw new Error(
      `Provider returned non-JSON response (${response.status}): ${responseText.slice(0, 200)}`,
    );
  }
